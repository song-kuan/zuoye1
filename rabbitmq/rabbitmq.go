...
func getURL()string{
    ...
    URL:="amqp://"+rabbitmq.User+":"+rabbitmq.Pwd+"@"+rabbitmq.Url+"/"+rabbitmq.Vhost
    return URL
}
...
//simple 模式下消费者
func (r *RabbitMQ) ConsumeSimple(orderService service.OrderService,productService service.ProductService) {
	//1.申请队列，如果队列不存在会自动创建，存在则跳过创建
	...
	go func() {
		for d := range msgs {
			...
			//插入订单
			if num,_:=productService.GetProductNumById(message.Productid);num<=1{
